...
public class AddServiceManager {
    ...
    public void addSectionToPath() {
        ...
        sectionsInPath.add(sectionPath_new);
        sectionPath_new = new SectionPath();
    ...
    public List<SectionPath> filterSectionPath() {
        ...
        for (int i = 0; i < sectionsInPath.size(); i++) {
            SectionPath get = sectionsInPath.get(i);
    ...
    public List<JobPath> filterJopPath() {
        ...
        for (int i = 0; i < jobInPath.size(); i++) {
            JobPath get = jobInPath.get(i);
    ...
    public void deleteJobPath() {
        ...
        jobInPath.remove(selectedJobPath);
        selectedJobPath = new JobPath();
    }
    ...
    public void deleteSelectedDepartmentPath() {
        ...
        for (int i = 0; i < sectionsInPath.size(); i++) {
            SectionPath get = sectionsInPath.get(i);
            if (get.getDepartmentId() == selectDepartmentPath.id) {
                sectionsInPath.remove(get);
                for (int j = 0; j < jobInPath.size(); j++) {
                    JobPath get1 = jobInPath.get(j);
                    if (get1.getSectionID() == get.getId()) {
                        jobInPath.remove(get1);
                    }
                }
            }
        }
    ...
    public void deleteSelectedSectionPath() {
        ...
        sectionsInPath.remove(selectSectionPath);
        for (int i = 0; i < jobInPath.size(); i++) {
            JobPath get = jobInPath.get(i);
            if (get.getSectionID() == selectSectionPath.getId()) {
                jobInPath.remove(get);
